...
int main(int argc, char**argv)
{
  ...
  int npts_ext = 976;
  double *output_pk = malloc(npts_ext*sizeof(double)); // this must be at least of length nk elements
  double h; 
  ...
  if (which_2pt ==1)
	...
	{
	  double k_out = pow(10.,k_m_ext[n]); 
	  if (k_out > 0.01 && k_out < 1.)  // only print out the k modes that are emulated
	    fprintf(fp,"%e %e\n", k_out, output_pk[n]);  
    ...
    {
      double *k_unlogged = malloc(npts_ext*sizeof(double));
      for (n = 0; n < npts_ext; n++)
	k_unlogged[n] = pow(10.,k_m_ext[n]);
      ...
      pk_to_xi(npts_ext, k_m_ext, output_pk, r_out, xi_out);
      ...
      free(r_out); free(xi_out);      free(k_unlogged); 
